...
\begin{itemize}
    ...
    \item \textbf{Control/Tracking}: This module is responsible for tracking the generated path and ensuring the tractor-trailer system follows it safely without collisions.
\end{itemize}

From the modules mentioned above, the ones that are implemented in this work are mainly 
the path planning and path tracking modules, however, in order to test and develop the system, 
...
path tracker which will then track it by computing the necessary angular and linear velocities while ensuring the 
...
The following flowchart \ref{fig:plannerfc} shows the logic of the planner, starting with 
...
Algorithm \ref{alg:planner} shows the detailed pseudocode for the planner. Starting 
...
The Dubins path creation is more mathematically complex to represent simply in pseudocode. 
...
\paragraph{}A path is nothing if it cannot be followed so, a Path tracker or controller capable of 
attending to this system's needs is required. This tracker will 
receive as input the generated path by the planner and will try to follow it. 
This is achieved in two steps, first, having the current positions in mind, 
it will choose the next subgoal in the path. Since the tracker is 
...
\label{sec:collision}
The collision avoidance aspect of the tracker is of the utmost importance 
in real world scenarios when dealing with possible workers in the field. To achieve 
this safety requirement, the tracker is equipped with a collision detection algorithm 
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\textwidth]{highlevelarch.png}
