...
In figure \ref{fig:nav2_stack} it is possible to see the different components of the \gls{NAV2} stack as well as their 
relations with one another. The BT is the Behaviour tree 
...
the data collected by the robot's sensors. Some other essencial elements are also the global and local 
...
equipped with a NVIDIA Jetson Agx Xavier as the main computer, a LIDAR and wheel encoders. 
...
It is possible to verify, in Figure \ref{fig:voronoi_graph1}, that the graph isn't exactly perfect, as it doesn't 
...
The last step to finish the planner is developing the recovery hybrid A* component. The previous 
...
time but also lead to very small adjustments in paths which could make them unfollowable by the tracker, 
...
the trackers' job of following the planned path.
...
velocity commands for the robot to track the planned path. In the \gls{NAV2} stack, 
to the velocity commands have a specific type and there's only the need to change the linear 
and angular atributes of the message and return it. The trackers' implementation, as a controller 
plugin, is similar to the planner's in the sense that it is a C++ class that 
implements the tracker interface that has only a few methods that need to be implemented. 
The first method is the setPlan method. As the name suggests, this method receives the 
global plan created by the planner and stores it for when the tracker may be called to 
produce velocity values. Since this tracker, when in reverse, uses the trailer as the 
...
for its task. Since the used tracker is the pure pursuit controller, the first step is 
...
tracker to follow the path while ensuring that the trailer doesn't jackknife.
...
only caveat being that since the tracker is using the local costmap for positioning, 
